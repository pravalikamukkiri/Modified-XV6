...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  //assing ticks to p->ctime 
  p->ctime = ticks;
  //initiaizing process rtime,iotime,etime with 0
  p->rtime=0;
  p->iotime=0;
  p->etime=0;
  p->n_run=0;
...
//waitx syscall
//it takes two arguments which are pointers to the integer values and then
//assign wait time and run time to those pointer values respectively.
...
waitx(int *wtime, int *rtime)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        // Found one.
        //assigning process wait time and run time to these values
        *rtime = p->rtime;
...
cps()
{
  ...
  // Enable interrupts on this processor.
    ...
    {
        if(p->state == SLEEPING)
            cprintf("%d      %d    SLEEPING     %d     %d     %d    %d   %d   %d   %d   %d   %d\n", p->pid,p->priority,p->rtime,p->wtime,p->n_run,p->queue,p->ticks[0],p->ticks[1],p->ticks[2],p->ticks[3],p->ticks[4]);
