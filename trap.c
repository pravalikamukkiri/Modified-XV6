...
trap(struct trapframe *tf)
{
  ...
  case T_IRQ0 + IRQ_TIMER:
    if(cpuid() == 0){
      ...
      wakeup(&ticks);
      release(&tickslock);
      //updating rtime or iotime of a process
      if(myproc()){
        if(myproc()->state == RUNNING){
          //if process state is running then update rtime 
          myproc()->rtime++;
          myproc()->wtime=0;
        }
        else if(myproc()->state == SLEEPING){
          //if process state is sleeping then update iotime
          myproc()->iotime++;
          //myproc()->wtime=ticks- myproc()->stime;
        }
